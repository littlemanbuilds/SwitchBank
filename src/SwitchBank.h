...
public:
    ...
    {
        uint32_t value;   ///< Stable packed value after last commit.
        uint32_t changed; ///< Bits that toggled on last commit.
        uint32_t rising;  ///< Bits that rose 0->1 on last commit.
        uint32_t falling; ///< Bits that fell 1->0 on last commit.
        uint32_t t_ms;    ///< Timestamp (ms) of last commit.
        uint32_t seq;     ///< Commit sequence counter.
    };
    ...
    bool update(uint32_t now_ms) override
    {
        ...
        const uint32_t raw = readPackedOnce(); ///< logical packed (post-polarity).
     ...
     */
    [[nodiscard]] uint32_t value() const noexcept override
    {
     ...
     * @return Packed bitmask of current value.
     */
    [[nodiscard]] uint32_t peekValue() const noexcept override { return current_; }

    /**
     * @brief Previous committed value.
     * @return Packed bitmask of previous value.
     */
    [[nodiscard]] uint32_t prevValue() const noexcept override { return previous_; }
     ...
     */
    [[nodiscard]] uint32_t changedMask() const noexcept override { return current_ ^ previous_; }

    /**
     * @brief Bits that rose 0→1 on the last commit.
     * @return Packed rising-edge mask.
     */
    [[nodiscard]] uint32_t risingMask() const noexcept override { return (~previous_) & current_; }

    /**
     * @brief Bits that fell 1→0 on the last commit.
     * @return Packed falling-edge mask.
     */
    [[nodiscard]] uint32_t fallingMask() const noexcept override { return previous_ & (~current_); }
     ...
     */
    bool isOn(uint8_t i) const noexcept
    {
        if (i >= N)
        {
            return false;
     ...
     */
    bool rose(uint8_t i) const noexcept
    {
        if (i >= N)
        {
            return false;
     ...
     */
    bool fell(uint8_t i) const noexcept
    {
        if (i >= N)
        {
            return false;
     ...
     */
    void setActiveLowMask(uint32_t mask) noexcept
    {
     ...
     */
    uint32_t activeLowMask() const noexcept { return effectiveMask(); }

    /**
     * @brief Timestamp of the last commit.
     * @return Milliseconds since time source epoch.
     */
    uint32_t lastCommitMs() const noexcept { return last_commit_ms_; }

    /**
     * @brief Number of commits since the last sync().
     * @return Commit counter.
     */
    uint32_t changeCount() const noexcept { return change_count_; }
    ...
    typename switchbank::compat::enable_if<(N <= 8), uint8_t>::type value8() const noexcept
    {
     ...
     */
    template <typename Dummy = void>
    typename switchbank::compat::enable_if<(N <= 8), uint8_t>::type changedMask8() const noexcept
    {
...
private:
    ...
    struct BitState
    {
        uint32_t last_change_ms{0}; ///< Millis when the input last toggled.
    ...
    static constexpr uint32_t compileTimeMask() noexcept
    {
        ...
        return hasCompileTimePolarity()
                   ? ((N == 32) ? static_cast<uint32_t>(PolarityMask)
                                : (static_cast<uint32_t>(PolarityMask) & ((1u << N) - 1u)))
                   : 0u;
    }
     ...
     */
    static constexpr uint32_t defaultActiveLowMaskRuntime() noexcept
    {
     ...
     */
    bool readOnePhysical(uint8_t idx) const noexcept
    {
        if (read_pin_fn_)
        {
     ...
     */
    bool commitIfChanged(uint32_t next, uint32_t now_ms) noexcept
    {
        if (next != current_)
        {
            previous_ = current_;
    ...
    // ---- Config / hooks ---- //
    uint8_t keys_[N]{};              ///< Key identifiers (pins or expander addresses).
    ...
    // ---- Timing / policy ---- //
    uint16_t debounce_ms_{0};                      ///< Debounce window in milliseconds.
    uint16_t min_poll_ms_{0};                      ///< Minimum interval between reads (0 disables).
    LatchMode latch_mode_{LatchMode::ManualClear}; ///< Changed flag policy.

    // ---- Polarity ---- //
    uint32_t active_low_mask_{defaultActiveLowMaskRuntime()}; ///< Runtime active-low mask (ignored if compile-time mask is set).

    // ---- Per-bit debounce state ---- //
    BitState bits_[N]; ///< Bitwise debounce state (O(N) RAM).

    // ---- Commit state ---- //
    uint32_t current_{0};         ///< Current committed packed value.
    ...
    // ---- Scan control ---- //
    uint32_t last_poll_ms_{0}; ///< Timestamp of last hardware scan (ms).
};
